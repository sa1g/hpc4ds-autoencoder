...
//     public:
//         Iterator(Dataloader *dataloader, int current_batch) : _dataloader(dataloader), _current_batch(current_batch) {}

//         // Dereference operator to get the current batch
//         Eigen::Tensor<float, 3> operator*()
//         {
//             // Ensure current_batch is within bounds
//             if (_current_batch >= _dataloader->num_batches)
//             {
//                 throw std::out_of_range("Iterator out of range");
//             }

//             Eigen::Tensor<float, 3> batch = _dataloader->get_batch();

//             // Move to the next batch index
//             _dataloader->_batch_start_index += 1;

//             // If we reach the end of the dataset, reset the batch index
//             if (_dataloader->_batch_start_index > _dataloader->num_batches)
//             {
//                 _dataloader->_batch_start_index = 0;
//             }
...
public:
    ...
    //     bool operator!=(const Iterator &other)
    //     {
    //         return _current_batch < _dataloader->num_batches;
    //     }
    // };
