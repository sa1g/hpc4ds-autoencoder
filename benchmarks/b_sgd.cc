...
template <typename Layer> void sgd(Layer &layer, float learning_rate) {
...
template <typename... Layers> void sgd(float learning_rate, Layers &...layers) {
...
static void BM_SGDUpdate(benchmark::State &state) {
  ...
  for (auto _ : state) {
    ...
    const double layer1_elements =
      static_cast<double>(input_dim) * hidden_dim + hidden_dim;
    const double layer2_elements =
      static_cast<double>(hidden_dim) * output_dim + output_dim;
    const double bytes_processed =
      4.0 * (layer1_elements + layer2_elements) * sizeof(float);
